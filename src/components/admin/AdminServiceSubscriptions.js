        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="bg-white/10 text-white hover:bg-white/20 border border-white/20 px-4 py-2 rounded-lg font-medium flex items-center"
            >
        ...
        <div className="p-6">
          ...
          {filteredSubscriptions.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-white/80">Aucun abonnement trouvé</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSubscriptions.map(subscription => (
                <div
                  key={subscription.id}
                  className="glass-card hover:bg-white/15 transition-shadow overflow-hidden"
                >
                  <div className={`${subscription.isActive ? 'bg-green-600/80' : 'bg-white/10'} p-4`}>
                    <div className="flex justify-between items-start">
                      <h3 className="text-lg font-semibold text-white">{subscription.name || 'Abonnement sans nom'}</h3>
                      {subscription.isActive ? (
                        <span className="badge-active text-xs px-2 py-1 rounded-full">Actif</span>
                      ) : (
                        <span className="badge-expired text-xs px-2 py-1 rounded-full">Inactif</span>
                      )}
                    </div>
                  ...
                  <div className="p-4">
                      ...
                      {subscription.accessType === 'account' ? (
                        <div className="space-y-2">
                          <div>
                            ...
                            <p className="text-white/80 font-medium">{subscription.email}</p>
                          </div>
                          <div>
                            <p className="text-xs text-white/50">Mot de passe</p>
                            <p className="text-white/80 font-medium">{subscription.password}</p>
                          </div>
                        </div>
                      ) : (
                        <div>
                          <p className="text-xs text-white/50">Lien d'invitation</p>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div>
                        <p className="text-xs text-white/50">Prix</p>
                        <p className="text-white/80 font-medium">{subscription.price.toFixed(2)} €</p>
                      </div>
                      <div>
                        <p className="text-xs text-white/50">Durée</p>
                        <p className="text-white/80 font-medium">{subscription.duration} jours</p>
                      </div>
                      <div>
                        <p className="text-xs text-white/50">Utilisateurs</p>
                        <p className="text-white/80 font-medium">{subscription.userCount} / {subscription.maxUsers}</p>
                      </div>
                      <div>
                        <p className="text-xs text-white/50">Expiration</p>
